package com.lx862.qomc;

import com.lx862.qomc.command.AbstractCommandManager;
import com.lx862.qomc.command.MinecraftCommandManager;
import com.lx862.qomc.config.DemoConfig;
import com.lx862.qomc.config.MainConfig;
import com.lx862.qomc.util.ModInfo;
import com.lx862.qomc.util.QconfUtil;
import com.lx862.qomc.xplat.Platform;
import com.mojang.brigadier.CommandDispatcher;
import com.mojang.brigadier.builder.LiteralArgumentBuilder;
import folk.sisby.kaleido.lib.quiltconfig.api.Config;
import folk.sisby.kaleido.lib.quiltconfig.impl.util.ConfigsImpl;

//? if 1.16.5 {
/*import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
*///? } else {
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
//? }

import java.util.*;

public class Qomc {
    public static String MOD_ID = "qomc";
    //? if 1.16.5 {
    /*public static final Logger LOGGER = LogManager.getLogger("QoMC");
    *///? } else {
    public static final Logger LOGGER = LoggerFactory.getLogger("QoMC");
    //? }

    public static void init() {
        MainConfig.init();
        DemoConfig.init();

        Platform.INSTANCE.registerCommand(commandDispatcher -> registerCommand(new MinecraftCommandManager(), commandDispatcher, false));
    }

    public static <S> void registerCommand(AbstractCommandManager<S> commandManager, CommandDispatcher<S> dispatcher, boolean forClientSide) {
        boolean unifiedNode = MainConfig.INSTANCE.unifiedConfigCommand.value();

        Map<ModInfo, List<Config>> configGroup = probeModConfigs(forClientSide);
        List<LiteralArgumentBuilder<S>> nodeToRegister = new ArrayList<>();

        for (Map.Entry<ModInfo, List<Config>> modConfigEntry : configGroup.entrySet()) {
            ModInfo belongingMod = modConfigEntry.getKey();
            List<Config> modConfigs = modConfigEntry.getValue();
            if(belongingMod.hidden()) continue;

            nodeToRegister.add(ConfigCommands.buildModNode(commandManager, belongingMod, belongingMod.id() + (unifiedNode ? "" : "_config") + (forClientSide ? "_client" : ""), modConfigs));
        }

        if(unifiedNode) {
            LiteralArgumentBuilder<S> rootNode = commandManager.literal("config");
            for(LiteralArgumentBuilder<S> commandNode : nodeToRegister) {
                rootNode.then(commandNode);
            }

            dispatcher.register(rootNode);
        } else {
            for(LiteralArgumentBuilder<S> commandNode : nodeToRegister) {
                dispatcher.register(commandNode);
            }
        }
    }

    private static Map<ModInfo, List<Config>> probeModConfigs(boolean forClientSide) {
        Map<ModInfo, List<Config>> configGroup = new HashMap<>();

        for(Config config : ConfigsImpl.getAll()) {
            boolean configMatchSide = config.id().equals("client") == forClientSide;
            if(!configMatchSide) continue;

            String configId = QconfUtil.getFamilyOrId(config);
            List<String> probeIds = Arrays.asList(
                    configId,
                    configId.replace("-", ""),
                    configId.replace("_", ""),
                    configId.replace("_", "-"),
                    configId.replace("-", "_")
            );

            boolean modFound = false;
            for (String id : probeIds) {
                if (Platform.INSTANCE.modLoaded(id)) {
                    List<Config> configs = configGroup.computeIfAbsent(Platform.INSTANCE.getModInfo(id), k -> new ArrayList<>());
                    configs.add(config);
                    modFound = true;
                    break;
                }
            }
            if(modFound) continue;

            Qomc.LOGGER.warn("[QoMC] Found config with id {}, but cannot match it with any mod!", configId);
        }
        return configGroup;
    }
}
